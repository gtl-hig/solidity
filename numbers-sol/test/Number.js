...
describe("Numbers", function () {
  ...
  describe("Deployment", function () {
    it("Should set the right initial state", async function () {
      ...
      expect(await num.getNumber()).to.equal(0);
    });

    it("Should set the right owner", async function () {
      const { num, owner } = await loadFixture(deployNumberFixture);

      expect(await num.owner()).to.equal(owner.address);
    });
  ...
  describe("setNumber", function () {
    it("Should set number properly", async function () {
      ...
      expect(await num.getNumber()).to.be.equal(42);
    });
  });
